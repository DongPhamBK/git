...
#!/usr/bin/python
# -*- coding: utf8 -*-
SQUARE_SIZE = (35, 35) #khung ngoài cùng của trò chơi
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, settings, show=True, fps=1000):# tốc độ rắn và đồ hoạ
        ...
        self._mutation_bins = np.cumsum([self.settings['probability_gaussian'], # tỉ lệ lai ghép
    ...
    def update(self) -> None:
        ...
        # Cá nhân hiện tại đã chết
            ...
            if (self.current_generation > 0 and self._current_individual == self._next_gen_size) or\
                ...
                print(self.settings)
                print('======================= Thế hệ thứ {} ======================='.format(self.current_generation))
                print('----Năng lực tối đa:', self.population.fittest_individual.fitness)
                print('----Điểm tốt nhất:', self.population.fittest_individual.score)
                print('----Năng lực trung bình:', self.population.average_fitness)
    ...
    def _crossover(self, parent1_weights: np.ndarray, parent2_weights: np.ndarray,
        ...
        # Crossover nhị phân một điểm (SPBX)
            ...
            raise Exception('Không thể xác định lai chéo hợp lệ dựa trên xác suất')
